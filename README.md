...
在网页中**长按**你设定的键，弹出 Chrome 常用快捷键速查面板；松手即关闭。支持自动识别 **Mac** / **Windows** 键位说明，也可在选项中固定为某一种布局。

## 功能概览

- **速查面板**：在任意普通网页（`http` / `https`）上通过长按触发键显示浮层，列出分类快捷键。
- **可配置**：触发键（如左/右 Alt、左/右 Shift、右 Ctrl）、长按时长（200～5000 ms，默认 800 ms）、快捷键列表为「自动 / 始终 Mac / 始终 Windows」。
